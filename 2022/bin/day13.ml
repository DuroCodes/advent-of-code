...
module Day13 : Day = struct
  type packet = PacketInt of int * packet | PacketList of packet * packet | PacketNull
  ...
  let parse_input t =
    ...
    |> List.map ~f:(fun x -> (parse_line (List.hd_exn x), parse_line (List.nth_exn x 1)))
    |> fun x -> Input x
  ...
  let solve_part1 (Input input) =
    input |> List.map ~f:compare_packets
    |> List.filter_mapi ~f:(fun i x -> if x = -1 then Some (i + 1) else None)
    |> List.fold ~init:0 ~f:( + )
    |> fun x -> AnswerInt x

  let solve_part2 (Input input) =
    let dp2 = PacketList (PacketList (PacketInt (2, PacketNull), PacketNull), PacketNull) in
    let dp6 = PacketList (PacketList (PacketInt (6, PacketNull), PacketNull), PacketNull) in
    ...
    |> List.filter_mapi ~f:(fun i x ->
           if compare_packets (x, dp2) = 0 || compare_packets (x, dp6) = 0 then Some (i + 1)
           else None)
    |> List.fold ~init:1 ~f:( * )
    |> fun x -> AnswerInt x
