...
module Day03 : Day = struct
  ...
  let item_in_both (comp1, comp2) =
    let common =
      List.filter comp1 ~f:(fun x -> List.exists comp2 ~f:(Char.equal x))
    in
    match common with [] -> '0' | hd :: _ -> hd
  ...
  let item_in_sacks sacks =
    let items =
      List.map sacks ~f:(fun (comp1, comp2) -> List.append comp1 comp2)
    in
    ...
    match common_items with [] -> '0' | hd :: _ -> hd
